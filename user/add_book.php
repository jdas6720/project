...
if (isset($_FILES["pdf_path"]["name"])){
  $fileName = date('Y-m-d-h-i-s')."_".basename($_FILES["pdf_path"]["name"]);
}
...
if(isset($_POST["submit"]) && isset($_FILES["pdf_path"]["name"])  && isset($_FILES["image"]["name"])){
    ...
    if (!file_exists($targetFilePath) && !file_exists($targetFilePath2) ) {
        if(in_array($fileType, $allowTypes) && in_array($fileType2, $allowTypes2)){
            ...
            if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath) && move_uploaded_file($_FILES["book_img"]["tmp_name"], $targetFilePath2)){
                ...
                $insert = $conn->query("INSERT into books (book_name,pdf_path,image ) VALUES ('$name','".$fileName."', '".$fileName2."')");
                if($insert){
                  ...
                  <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post"  enctype="multipart/form-data">
                    <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <input name="pdf_path" type="file" class="form-control" id="exampleFormControlInput1" placeholder="image body" >
                      </div>
                      <div class="form-group">
                        <label for="exampleFormControlInput1">Upload Image</label>
                        <input name="image" type="file" class="form-control" id="exampleFormControlInput1" placeholder="image" >
                      </div>
                      <div class="form-group">
